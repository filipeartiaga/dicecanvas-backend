...
import { GetAllLogController } from '../../../presentation/controllers/log/get-all'
import { DbGetAllLog } from '../../../data/usecases/log/db-get-all-logs'
import { GetAllLogsMongoRepository } from '../../../infra/db/mongodb/log/get-all-logs'
...
export const makeGetAllLogController = (): GetAllLogController => {
  ...
  const getAllLogMongoRepository = new GetAllLogsMongoRepository()
  const dbGetAllLogs = new DbGetAllLog(getAllLogMongoRepository)
  const authenticatedValidator = new AuthenticatedValidatorAdapter(dbGetUser, userDecoderAdapter)
  ...
  const controller = new GetAllLogController(authenticatedValidator, dbGetAllLogs, dbGetCharacterSheet)
  return controller
}
